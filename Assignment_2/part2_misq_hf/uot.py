...
def parse_response(text, possibilities):
    ...
    # fallback: grab any question-looking string
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                print(f"gen_questions failed (attempt {attempt}/{max_retries}), retrying")
                time.sleep(1)
            else:
                print(f"gen_questions failed after {max_retries} retries: {err}")
                return []
    print(f"gen_questions failed with unknown error: {last_err}")
    return []
