...
from mcts import misq_search, propagate_feedback
from embedding import assign_cluster
...
def parse_yn(text):
    ...
    if t.startswith("no"):
        return "NO"
    # word boundary to avoid matching "normal", "know" etc
...
Ask yes/no questions starting with 'Is X'. Ask one question at a time."""
    ...
    for turn in range(T):
        ...
        conv.append({"turn": turn + 1, "question": question, "response": resp})
...
def run_misq(q_llm, a_llm, sample, omega, domain, config, shared_root):
    sid, idx, self_repo, target = sample
    T = config.get("max_turns", 6)
    m = config.get("n_questions", 3)
    n_iter = config.get("n_iter", 10)
    mcts_c = config.get("mcts_c", 0.2)
    ...
    qgc = {"count": 0}
    success = False
    info_turns = int(config.get("delta", 0.6) * T)

    print(f"\n--- [MISQ] Sample #{idx}: target='{target}' ---")
    print(f"  self_repo: {self_repo[:80]}...")
    print(f"  |O| = {len(cur.possibilities)}, T = {T}, info_turns = {info_turns}")
    ...
    for turn in range(T):
        ...
        if turn < info_turns and not cur.is_terminal():
            best_q = misq_search(q_llm, cur, m=m, domain=domain,
                                 qgc=qgc, n_iter=n_iter, c=mcts_c)
            if best_q is None:
                print("    [no question from MCTS, targeting]")
                question, guessed = make_guess(q_llm, cur.possibilities, domain)
                used_tree = False
            else:
                question = best_q.question
                ...
                used_tree = True
                print(f"    Q: {question[:80]}...")
                print(f"    visits={best_q.visits} total_r={best_q.total_reward:.3f}")
        else:
            question, guessed = make_guess(q_llm, cur.possibilities, domain)
            used_tree = False
            if question is None:
        ...
        conv.append({
            ...
            "question": question,
            "response": resp,
            "phase": "mcts" if turn < info_turns else "targeting"
        })
...
def run_misq_hf(q_llm, a_llm, sample, omega, domain, config,
    ...
    emb = embedder.encode(self_repo)
    cluster_id = assign_cluster(emb, clusters, threshold=tau)
    ...
    print(f"\n--- [MISQ-HF] Sample #{idx}: target='{target}' ---")
    print(f"  self_repo: {self_repo[:80]}...")
    print(f"  cluster_id={cluster_id}, total clusters so far={len(clusters)}")
    print(f"  |O| = {len(cur.possibilities)}, T = {T}, info_turns = {info_turns}")
    ...
    for turn in range(T):
        ...
        if turn < info_turns and not cur.is_terminal():
            ...
            if best_q is None:
                ...
                guessed = None
                used_tree = True
                fb = best_q.cluster_bonus.get(cluster_id, 0)
                print(f"    Q: {question[:80]}...")
                print(f"    visits={best_q.visits} total_r={best_q.total_reward:.3f} fb_bonus={fb:.3f}")
        else:
            question, guessed = make_guess(q_llm, cur.possibilities, domain)
            used_tree = False
            if question is None:
        ...
        conv.append({
            ...
            "question": question,
            "response": resp,
            "phase": "mcts_hf" if turn < info_turns else "targeting"
        })
    ...
    if success:
        print(f"    propagating feedback to cluster {cluster_id}")
        propagate_feedback(cur, cluster_id, target=target, beta=beta, gamma=gamma)
        if cluster_id not in clusters:
            clusters[cluster_id] = [emb]
        else:
