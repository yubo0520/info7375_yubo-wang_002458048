...
try:
    ...
    _HAS_AGENTOPS_SDK = True
except ImportError:
    _HAS_AGENTOPS_SDK = False
    TracingCore = None
    try:
        import agentops
    except ImportError:
    ...
    class SpanProcessor(_OtelSpanProcessor):
        pass
...
def _get_agentops_client():
    """Return AgentOps client across API versions."""
    if agentops is None:
        return None

    get_client = getattr(agentops, "get_client", None)
    ...
    meta_client = getattr(agentops, "meta_client", None)
    if callable(meta_client):
        try:
            return meta_client()
        except Exception:
            return None
...
class AgentOpsTracer(BaseTracer):
    ...
    def __init__(self, *, agentops_managed: bool = True, instrument_managed: bool = True, daemon: bool = True):
        ...
        if AgentOpsServerManager is None and self.agentops_managed:
            logger.warning(
                "AgentOps server deps are missing (e.g. flask/setproctitle). "
                "Falling back to agentops_managed=False."
            )
            self.agentops_managed = False

        self._agentops_server_manager = AgentOpsServerManager(self.daemon) if AgentOpsServerManager else None
    ...
    def init_worker(self, worker_id: int):
        ...
        if self.agentops_managed:
            ...
            client_instance = _get_agentops_client()
            if not getattr(client_instance, "initialized", False):
                agentops.init()
    ...
    def get_langchain_callback_handler(self, tags: List[str] | None = None) -> LangchainCallbackHandler:
        ...
        client_instance = _get_agentops_client()
        api_key = None
        if getattr(client_instance, "initialized", False):
