...
The step numbering in this folder follows the assignment naming. The runnable materials here correspond to Steps 1, 2, 3, 5, and 6.
...
These `/runs/...` paths are Modal volume paths, not normal local folders.

For more detail on this separate lane, see `task56_iso_new/README.md`.
...
The latest detailed rerun/status snapshot is maintained in `quick_start_guide.md`.

At a high level:

- Part 1 is completed.
- Step 1 baseline runs were completed.
- Step 2 Qwen3.5 scaling runs were completed.
- Step 3 BIRD runs were completed for the main local sizes plus the DashScope path.
- Step 5 and Step 6 simplified-agent training/evaluation runs were completed in the separate `task56_iso_new/` lane.
